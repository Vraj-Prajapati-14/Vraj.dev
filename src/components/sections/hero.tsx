...
const headlineLines = [
  { text: 'APIs First.',     className: 'block text-[2.4rem] sm:text-5xl md:text-6xl lg:text-[4.5rem] text-[#F0EDE8]' },
  { text: 'Everything Else', className: 'block text-[2.4rem] sm:text-5xl md:text-6xl lg:text-[4.5rem] text-[#333333] italic' },
  { text: 'Follows.',        className: 'block text-[2.4rem] sm:text-5xl md:text-6xl lg:text-[4.5rem] gradient-text italic pb-2' },
]
...
export function Hero() {
  ...
  useEffect(() => {
    import('animejs').then(({ animate, stagger }) => {
      ...
      animate(headingRef.current.querySelectorAll('.hero-char'), {
        opacity: [0, 1],
        translateY: [44, 0],
        rotateX: [-90, 0],
        delay: stagger(18, { start: 250 }),
        duration: 750,
        ease: 'outExpo',
      })
    })
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full py-12 md:py-0">
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-16 items-center min-h-[88vh] pb-8 lg:pb-0">
          ...
          <div className="order-2 lg:order-1">
            ...
            {/* Headline — anime.js char-by-char animation */}
            <h1
              ref={headingRef}
              className="luxury-heading font-bold leading-[1.05] mb-6"
              style={{ perspective: '800px' }}
            >
              {headlineLines.map((line, li) => (
                <span key={li} className={line.className} aria-label={line.text}>
